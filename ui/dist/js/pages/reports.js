    ...
    getFromAPI({
        ...
        if (data.message?.message != null) {
            ...
            for (i = 0; i < message.length; i++) {
                var value = message[i]['period']
                var text = message[i]['period']
                if (value != null) {
                    $('#sel_date')
                        .append($('<option>', {
                            value: value,
...
function GetReport() {
    ...
    getFromAPI({
        ...
        columns = $.map(data.message, function (v, i) {
            return {
                'data': v, 'title': v.toUpperCase(), 'render': function (a, display, val, row, e, f) {
                    return val[v];
                }
            }
        });
...
async function getData(url, method) {
    ...
    try {
        const response = await fetch('/f3-adminlte/api/reports.php', { method: "GET" });
        if (!response.ok) {
                    ...
                    "success": function (json) {
                        if (json.message.Error != null) {
                            ...
                            dt = {
                                ...
                                "recordsTotal": json.message.result.length,
                                "recordsFiltered": json.message.result.length
                            }
                        }
